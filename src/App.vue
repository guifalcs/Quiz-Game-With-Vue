  ...
  <div>
    ...
    <template v-if="this.question">
      ...
      <template v-for="(answer, index) in this.answers" :key="index">
        <input :disabled="this.answerSubimitted" v-model="this.chosenAnswer" type="radio" name="options" :value="answer">
        <label v-html="answer"></label><br>
      </template>

      <button v-if="!this.answerSubimitted" @click="this.submitAnswer()" class="send" type="button">Send</button>

      <section v-if="this.answerSubimitted" class="result">
          <h4 v-if="this.chosenAnswer == this.correctAnswer" v-html="'&#9989;Correct!'"></h4>

          <h4 v-else v-html="'&#10060; Wrong answer, the correct is' + this.correctAnswer" ></h4>
...
export default {
  ...
  computed: {
    answers(){
      ...
      answers.splice(Math.round(Math.random()*answers.legnth),0, this.correctAnswer)
      return answers
    }
  },
  methods: {
